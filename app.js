...
async function processImage() {
    ...
    try {
        ...
        // Validate OCR words and split merged words
        const validatedWords = validateOCRWords(mergedWords);

        state.ocrData = { words: validatedWords };
        state.selectedWords.clear();

        debugLog('OCR found', validatedWords.length, 'words (after validation)');
...
// Common English words set for OCR validation (top ~3000 words + common suffixes)
const COMMON_WORDS = new Set([
    // Articles, prepositions, conjunctions
    'a','an','the','and','or','but','if','then','else','when','where','why','how','what','which','who','whom',
    'in','on','at','to','for','of','with','by','from','up','down','out','into','over','under','after','before',
    'between','through','during','about','against','among','around','above','below','behind','beside','beyond',
    // Pronouns
    ...
    'this','that','these','those','all','each','every','both','few','more','most','other','some','such','no','nor',
    'not','only','own','same','so','than','too','very','just','also','now','here','there','where','when','why','how',
    // Common verbs
    'be','am','is','are','was','were','been','being','have','has','had','having','do','does','did','doing','done',
    'will','would','shall','should','may','might','must','can','could','go','goes','went','gone','going','come',
    'comes','came','coming','get','gets','got','getting','make','makes','made','making','know','knows','knew',
    'known','knowing','think','thinks','thought','thinking','take','takes','took','taken','taking','see','sees',
    'saw','seen','seeing','want','wants','wanted','wanting','give','gives','gave','given','giving','use','uses',
    'used','using','find','finds','found','finding','tell','tells','told','telling','ask','asks','asked','asking',
    'work','works','worked','working','seem','seems','seemed','seeming','feel','feels','felt','feeling','try',
    'tries','tried','trying','leave','leaves','left','leaving','call','calls','called','calling','need','needs',
    'needed','needing','become','becomes','became','becoming','put','puts','putting','mean','means','meant',
    'meaning','keep','keeps','kept','keeping','let','lets','letting','begin','begins','began','begun','beginning',
    'show','shows','showed','shown','showing','hear','hears','heard','hearing','play','plays','played','playing',
    'run','runs','ran','running','move','moves','moved','moving','live','lives','lived','living','believe',
    'believes','believed','believing','hold','holds','held','holding','bring','brings','brought','bringing',
    'happen','happens','happened','happening','write','writes','wrote','written','writing','provide','provides',
    'provided','providing','sit','sits','sat','sitting','stand','stands','stood','standing','lose','loses','lost',
    'losing','pay','pays','paid','paying','meet','meets','met','meeting','include','includes','included','including',
    ...
    'understanding','watch','watches','watched','watching','follow','follows','followed','following','stop','stops',
    'stopped','stopping','create','creates','created','creating','speak','speaks','spoke','spoken','speaking',
    'read','reads','reading','allow','allows','allowed','allowing','add','adds','added','adding','spend','spends',
    'spent','spending','grow','grows','grew','grown','growing','open','opens','opened','opening','walk','walks',
    'walked','walking','win','wins','won','winning','offer','offers','offered','offering','remember','remembers',
    'remembered','remembering','love','loves','loved','loving','consider','considers','considered','considering',
    'appear','appears','appeared','appearing','buy','buys','bought','buying','wait','waits','waited','waiting',
    'serve','serves','served','serving','die','dies','died','dying','send','sends','sent','sending','expect',
    ...
    'require','requires','required','requiring','report','reports','reported','reporting','decide','decides',
    'decided','deciding','pull','pulls','pulled','pulling','develop','develops','developed','developing',
    // Common nouns
    'time','year','people','way','day','man','woman','child','children','world','life','hand','part','place',
    'case','week','company','system','program','question','work','government','number','night','point','home',
    'water','room','mother','area','money','story','fact','month','lot','right','study','book','eye','job',
    'word','business','issue','side','kind','head','house','service','friend','father','power','hour','game',
    'line','end','member','law','car','city','community','name','president','team','minute','idea','kid','body',
    'information','back','parent','face','others','level','office','door','health','person','art','war','history',
    'party','result','change','morning','reason','research','girl','guy','moment','air','teacher','force',
    'education','foot','boy','age','policy','process','music','market','sense','nation','plan','college','interest',
    'death','experience','effect','use','class','control','care','field','development','role','effort','rate',
    'heart','drug','show','leader','light','voice','wife','police','mind','difference','period','building',
    'action','authority','model','evidence','value','data','student','position','road','table','form','individual',
    'view','society','news','order','century','section','paper','group','ground','language','nature','window',
    'account','thought','figure','choice','summer','south','north','east','west','wall','response','attention',
    'sun','movement','blood','page','picture','dog','cat','bird','fish','tree','flower','grass','river','lake',
    'ocean','sea','mountain','hill','forest','garden','park','street','bridge','church','school','hospital',
    'store','shop','restaurant','hotel','office','farm','factory','station','airport','port','island','beach',
    'rock','stone','sand','snow','rain','wind','fire','earth','sky','star','moon','cloud','spring','fall',
    'winter','color','red','blue','green','yellow','black','white','brown','orange','purple','pink','gold',
    'silver','brother','sister','son','daughter','baby','family','husband','doctor','nurse','king','queen',
    'prince','princess','lord','lady','sir','master','captain','soldier','army','ship','boat','train','plane',
    'car','bus','truck','horse','cow','pig','sheep','chicken','goat','deer','bear','wolf','lion','tiger',
    'elephant','monkey','mouse','rabbit','fox','snake','frog','bee','ant','spider','food','bread','meat',
    'fruit','apple','orange','banana','grape','lemon','milk','cheese','butter','egg','rice','sugar','salt',
    'oil','tea','coffee','wine','beer','cake','pie','candy','ice','cream','soup','salad','sauce','meal',
    'breakfast','lunch','dinner','table','chair','bed','desk','door','window','floor','ceiling','roof','wall',
    'room','house','building','church','school','hospital','hotel','store','shop','farm','garden','park',
    'street','road','bridge','river','lake','sea','ocean','mountain','hill','forest','field','island','beach',
    // Common adjectives
    ...
    'sure','free','better','true','whole','real','best','low','late','hard','major','national','political',
    'social','certain','possible','personal','special','recent','clear','economic','international','strong',
    'full','available','likely','necessary','financial','military','current','federal','foreign','happy','nice',
    'beautiful','pretty','ugly','tall','short','fat','thin','rich','poor','fast','slow','hot','cold','warm',
    'cool','wet','dry','clean','dirty','dark','light','loud','quiet','soft','hard','easy','difficult','safe',
    'dangerous','healthy','sick','alive','dead','open','closed','empty','full','single','double','half','whole',
    'simple','complex','main','basic','common','normal','strange','usual','natural','physical','mental','moral',
    // Common adverbs
    'up','so','out','just','now','how','then','more','also','here','well','only','very','even','back','there',
    'down','still','in','as','too','when','never','really','most','on','why','about','over','such','through',
    'new','however','before','between','after','since','without','under','within','along','following','including',
    'today','always','often','sometimes','usually','already','ever','yet','again','once','twice','soon','later',
    'early','finally','perhaps','probably','certainly','definitely','actually','recently','suddenly','quickly',
    'slowly','carefully','easily','hardly','nearly','almost','quite','rather','enough','together','alone',
    'away','home','outside','inside','forward','backward','around','everywhere','nowhere','somewhere','anywhere',
    // Numbers
    'one','two','three','four','five','six','seven','eight','nine','ten','eleven','twelve','thirteen','fourteen',
    'fifteen','sixteen','seventeen','eighteen','nineteen','twenty','thirty','forty','fifty','sixty','seventy',
    'eighty','ninety','hundred','thousand','million','billion','first','second','third','fourth','fifth',
    // Contractions and common informal words
    "don't","doesn't","didn't","won't","wouldn't","can't","couldn't","shouldn't","isn't","aren't","wasn't",
    "weren't","haven't","hasn't","hadn't","i'm","i've","i'll","i'd","you're","you've","you'll","you'd",
    "he's","he'll","he'd","she's","she'll","she'd","it's","it'll","we're","we've","we'll","we'd",
    "they're","they've","they'll","they'd","that's","there's","here's","what's","who's","let's",
    // Religious/spiritual (common in reading materials)
    'god','lord','jesus','christ','spirit','holy','heaven','hell','angel','devil','soul','faith','prayer',
    'church','bible','sin','blessing','grace','mercy','salvation','resurrection','eternal','divine','sacred',
    'believers','believer','belief','believing'
]);

// Validate OCR words and flag suspicious ones
function validateOCRWords(words) {
    ...
    for (const word of words) {
        ...
        // Skip very short words or empty after cleaning
        if (text.length <= 1) {
    ...
    if (suspiciousCount > 0) {
        debugLog(`Found ${suspiciousCount} suspicious OCR words`);
    }

    return validatedWords;
}

// Check if a word is likely valid English
function isValidWord(text) {
    // Direct match in dictionary
    if (COMMON_WORDS.has(text)) return true;

    // Check without common suffixes
    const suffixes = ['ing', 'ed', 'er', 'est', 'ly', 's', 'es', 'tion', 'ness', 'ment', 'able', 'ible', 'ful', 'less', 'ous', 'ive'];
    ...
    for (const prefix of prefixes) {
        if (text.startsWith(prefix) && text.length > prefix.length + 2) {
            const stem = text.slice(prefix.length);
            if (COMMON_WORDS.has(stem)) return true;
        }
    }

    // Check for impossible consonant clusters (likely OCR error)
...
function trySplitMergedWord(text, originalWord) {
    ...
    for (let i = 3; i < text.length - 2; i++) {
        ...
        if (COMMON_WORDS.has(part1) && COMMON_WORDS.has(part2)) {
            // Found valid split - create two words with estimated bounding boxes
            const bbox = originalWord.bbox;
            const splitRatio = i / text.length;
            const splitX = bbox.x0 + (bbox.x1 - bbox.x0) * splitRatio;

            return [
                {
                    text: part1,
                    bbox: { x0: bbox.x0, y0: bbox.y0, x1: splitX, y1: bbox.y1 },
                    splitFrom: originalWord.text
                },
                {
                    text: part2,
                    bbox: { x0: splitX, y0: bbox.y0, x1: bbox.x1, y1: bbox.y1 },
                    splitFrom: originalWord.text
                }
            ];
        }
